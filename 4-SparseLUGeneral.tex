\section{General LU factorization for sparse matrix}
...
\begin{exm}
    For example, when we have $\mL$ and $G(\mL)$ as in Figure 
    \ref{fig::LGL}.
    \begin{figure}[H]
        \includegraphics[width=0.49\linewidth]{png/L.png}
        \hfill
        \includegraphics[width=0.49\linewidth]{png/GL.png}
        \caption{$\mL$ and corresponding $G(\mL)$}
        \label{fig::LGL}
    \end{figure}
    Assuming only $b_4,b_6\neq 0$, then the depth-first search 
    paths are as in Figure \ref{fig::LGL2}, where the red 
    points represent the edges in the search path.
    \begin{figure}[H]
...
\begin{alg}
    ...
    \begin{algorithm}[H]
        ...
        \Postcond{$\mL$ is lower triangular, $\mU$ is upper 
        triangular}
